...
describe("Integration: ingestion + analyzers", () => {
  it(
    "fetches revisions for a stable page and produces valid shape",
    async () => {
  ...
  it(
    "section differ extracts sections from revision content",
    async () => {
      ...
      const { sectionDiffer } = await import(
        "../../../analyzers/dist/src/index.js"
      );
  ...
  it(
    "revert detector identifies reverts in edit comments",
    async () => {
      const { revertDetector } = await import(
        "../../../analyzers/dist/src/index.js"
      );

      expect(revertDetector.isRevert("Reverted to previous version")).toBe(
        true
      );
      expect(
        revertDetector.isRevert("Undid revision 12345 by User (talk)")
      ).toBe(true);
      expect(revertDetector.isRevert("rvv")).toBe(true);
      expect(revertDetector.isRevert("Fixed typo in introduction")).toBe(false);
      expect(revertDetector.isRevert("Added new section on climate")).toBe(
        false
      );
    },
    { timeout: 5000 }
