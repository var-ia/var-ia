...
export async function runExplore(
  ...
  const server = createServer((req, res) => {
    ...
    const html = renderExplorePage(pageTitle, events, revisions, port);
...
.timeline-event { position: absolute; top: 0; height: 60px; cursor: pointer; opacity: 0.5; }
...
.export-link { color: var(--accent); text-decoration: none; font-size: 13px; padding: 6px 12px; border: 1px solid var(--border); border-radius: 6px; display: inline-block; }
...
<header>
  ...
  <span class="meta">${revisions.length} revisions · ${events.length} events · ${new Date(sortedRevs[sortedRevs.length - 1]?.timestamp ?? "").toLocaleDateString()}</span>
</header>
<nav>
...
<main>
  ...
  <div id="tab-timeline" class="tab">
    ${renderTimeline(sortedRevs, events)}
  </div>
  <div id="tab-revisions" class="tab">
    ${renderRevisionTable(sortedRevs, events)}
  </div>
  <div id="tab-summary" class="tab">
    ${renderSummary(events, eventCounts)}
  </div>
</main>
...
function renderSummary(events: EvidenceEvent[], counts: Record<string, number>): string {
  ...
  return `
    ...
    <div class="event-row"><span>Event types</span><span style="font-weight:600">${Object.keys(counts).length}</span></div>
